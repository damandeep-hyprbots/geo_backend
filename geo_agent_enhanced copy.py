...
Return ONLY the optimized body content HTML (without <body> tags), nothing else.
"""
    ...
    def _optimize_images_and_links(self, body_element, target_keyword: str) -> None:
        ...
        for link in links:
            ...
            if not link.get('title'):
                ...
                if link_text:
                    link['title'] = f"Learn more about {target_keyword}"
                else:
                    link['title'] = f"Click to learn more about {target_keyword}"
...
"""
    ...
    def _optimize_html(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            if elements['meta_description']:
                ...
                # Extract the content from the optimized meta description
                optimized_meta_soup = BeautifulSoup(optimized_meta, 'html.parser')
                meta_tag = optimized_meta_soup.find('meta')
                if meta_tag:
                    # Update the existing meta description content
                    elements['meta_description']['content'] = meta_tag.get('content', '')
                ...
                if elements['head']:
                    # Parse the meta description and add it to head
            ...
            for h_type, headings in optimized_headings.items():
                for i, new_heading in enumerate(headings):
                    if heading_index < len(all_headings):
                        ...
                        # Update heading content directly
            ...
            if schema_markup and elements['head']:
                # Parse the schema markup and add it to head
            ...
            if elements['body']:
                print("📝 Optimizing body content...")
                optimized_body_content = self._optimize_body_content(elements['body'], state['target_keyword'])
                
                if optimized_body_content:
                    # Replace body content with optimized version
                    new_body_soup = BeautifulSoup(optimized_body_content, 'html.parser')
                    new_body_content = new_body_soup.find('body')
                    if new_body_content:
                        # Clear existing body content and add optimized content
                        elements['body'].clear()
                        for child in new_body_content.children:
                            if child.name:  # Only add named tags, not text nodes
                                elements['body'].append(child)
                    print("✅ Body content optimized")
                
                # Optimize images and links in the body
                self._optimize_images_and_links(elements['body'], state['target_keyword'])
                print("✅ Images and links optimized")
                
                # Add internal linking and structured content
            ...
            with open('optimized_html.html', 'w') as f:
                f.write(optimized_html)

            
            print(f"✅ Successfully optimized HTML ({len(optimized_html)} characters)")
        ...
        try:
            ...
            for website in state['similar_websites']:
                ...
                if website.get('extracted_content') and  website.get('is_your_website', False):
                    url = website['url']
                    print(url)
                    orignal_document.append({
                        "text": state['current_html']
                    })
            ...
            print("hi"*30)
            print()
            with open('orignal_document_1.json', 'w') as f:
                json.dump(state['current_html'], f)
    ...
    def _finalize(self, state: AgentState) -> AgentState:
        ...
        final_result = {
            ...
            "optimized_html": state['best_html'],
...
def display_optimization_results(results: Dict[str, Any]) -> None:
    ...
    print(f"\nOptimized HTML Length: {len(results['optimized_html'])} characters")
    print(f"{'='*60}")
