#!/usr/bin/env python3
"""
Improved startup script for GEO Agent API Architecture with ngrok integration
"""
...
def start_ngrok_improved(port=8000):
    ...
    try:
        ...
        # Start ngrok with more detailed output and better configuration
        ngrok_process = subprocess.Popen(
            ['ngrok', 'http', str(port), '--log=stdout', '--log-level=info'],
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if tunnels.get('tunnels'):
                        ...
                        print(f"⏳ Waiting for tunnel to be ready... (attempt {attempt + 1}/15)")
                        time.sleep(2)
                else:
                    print(f"⏳ ngrok API not ready yet... (attempt {attempt + 1}/15)")
                    time.sleep(2)
            except requests.RequestException:
                print(f"⏳ ngrok API not responding... (attempt {attempt + 1}/15)")
                time.sleep(2)
        
        print("⚠️  ngrok started but couldn't get public URL after 15 attempts")
...
def start_fastapi_server(port=8000):
    """Start the FastAPI server for the new architecture"""
    print(f"🚀 Starting FastAPI Architecture Server on port {port}...")
    
    try:
        # Start uvicorn server with the new architecture
        ...
        if server_process.poll() is None:
            print(f"✅ FastAPI Architecture Server started on http://localhost:{port}")
...
def main():
    """Main function"""
    print("🌐 GEO Agent API Architecture Startup")
    print("=" * 50)
    print("🏗️  Architecture:")
    print("   Frontend → Score API → (score + md)")
    print("   Frontend → AI API → (url+html+md+score)")
    print("   Frontend → SSE API → (optimized html, optimized score)")
    print("=" * 50)
    ...
    print("\n" + "=" * 60)
    print("🎉 GEO Agent API Architecture is running!")
    print("=" * 60)
